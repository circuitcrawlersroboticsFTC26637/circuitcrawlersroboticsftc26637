...
// --- TYPE GUARD ---
/**
 * Type guard to narrow LogoItem to the node-based type.
 * This function tells TypeScript that if 'true' is returned, 'item' has the 'node' property.
 */
...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    // --- RENDER LOGO ITEM WITH TYPE SAFETY ---
    const renderLogoItem = useCallback(
      (item: LogoItem, key: React.Key) => {
        ...
        const isNode = isNodeLogoItem(item); // Use the defined type guard
        ...
        const content = isNode ? (
          <span
            ...
            // Type-safe access: item is guaranteed to be NodeLogoItem here
          ...
          <img
            ...
            // Type-safe access: item is guaranteed to be ImageLogoItem here
            src={item.src}
            srcSet={item.srcSet}
            sizes={item.sizes}
            width={item.width}
        ...
        // Type-safe access across the union type
        const itemAriaLabel = isNode
          ? (item.ariaLabel ?? item.title)
          : (item.alt ?? item.title);

        // 'href' exists on both union members, so access is safe
